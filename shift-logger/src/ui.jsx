...
export const Btn = ({ children, onClick, variant = 'solid', disabled, style, className = '', ...p }) => {
  ...
  const v = {
    ...
    ghost: { background: 'rgba(255,255,255,.5)', color: C.muted, border: '1px solid rgba(124,41,255,.18)' },
      ...
      {type === 'segment' && (
        <div style={{ display: 'flex', gap: 6 }}>
          {options.map(o => { const on = value === o; return (
            <button key={o} onClick={() => onChange(o)} className="rounded-xl" style={{ flex: 1, padding: '9px 4px', fontSize: 12, fontWeight: 700, cursor: 'pointer', border: on ? 'none' : '1px solid rgba(124,41,255,.18)', background: on ? C.violet : 'rgba(255,255,255,.5)', color: on ? '#fff' : C.muted }}>{o}</button>); })}
        </div>
      )}
      {type === 'multiselect' && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
          {options.map(o => { const arr = Array.isArray(value) ? value : []; const on = arr.includes(o); return (
            <button key={o} onClick={() => onChange(on ? arr.filter(x => x !== o) : [...arr, o])} className="rounded-full"
              style={{ padding: '7px 12px', fontSize: 11.5, fontWeight: 700, cursor: 'pointer', border: on ? 'none' : '1px solid rgba(124,41,255,.18)', background: on ? C.violet : 'rgba(255,255,255,.5)', color: on ? '#fff' : C.violetDim }}>{on ? '✓ ' : ''}{o}</button>); })}
        </div>
      )}
    </div>
  );
