...
- **Reminders.** Rule-based follow-throughs, always scoped to the **profile** (any shift, any person) and persistent until resolved (Snooze = 5 min). Rules live in `src/config.js` (`REMINDERS`: per-rule delay, condition, auto-clear activity, buttons). Requires re-running `supabase/schema.sql` once (adds the `reminders` table).
  ...
  3. **Project delivered** or **Shared to client (chat)** → 15 h later: follow up on **that specific item** (draft/files/shared elements) with **that client** — the reminder names both. **Auto-cleared** if a **Revision assigned** for the **same client + same project** is logged first (the client came back — no follow-up needed). Buttons: **Responded** (client already replied) · **Followed up**.
  4. **Files Assigned to Designer** → **immediately**: suggest an upsell to that client (the CSR knows what to offer). **Auto-cleared** if an **Upsell** for that client is logged first. Buttons: **No need** · **Upsell done**.
  5. **New inquiry** → 25 min later: send the prospect their **1st follow-up** (shows what they wanted). **Auto-cleared** if a **New order** for that client is logged first (they converted). Buttons: **In discussion** · **Followed up** · **Order taken**.
  6. **Lead follow-up chain** — logging the **1st** → 12 h later remind the **2nd**; the **2nd** → 24 h later remind the **3rd**; the **3rd** → 48 h later remind the **4th & last**. A "4th+" entry books nothing. **Auto-cleared** if a **New order** for that client is logged first. Buttons: **In discussion** · **Followed up** · **Order taken**.
  7. **New order** → **30 min later**: assign the order to a designer. **Auto-cleared** if **Order Assigned to Designer** for the **same project** is logged first. Buttons: **Next shift** (reminds again in 8 h) · **Assigned**.
  8. **New order marked "Direct Order"** (second reminder) → **immediately**: potential upsell — what's missing / what else would help, **especially a Website** (the order's service shows as context). Chat orders get no upsell nudge. **Auto-cleared** if an **Upsell** for that client is logged first. Buttons: **No need** · **Upsell done**.
  9. **Revision assigned to designer** → after the time the CSR types in the form's **"Remind me in"** box (`30m`, `2h`, `5 h`, …): check whether the revision is done. **Left blank ⇒ defaults to 24 h** (always books a reminder). Buttons: **Follow-up done** · **Revision done** (plus Snooze).
  10. **Offer** → button-driven follow-up chain: **+2 h** the 1st follow-up; tapping **1st F/U done** books the 2nd (**+16 h** from the tap); **2nd F/U done** books the 3rd (**+36 h**); **3rd F/U done** ends the chain. **Order placed** at any stage ends it too. Also **auto-cleared** at any stage if a **New order** for that client is logged.
  ...
  12. **Custom reminder** → a manual reminder the CSR sets: a **heading**, a **note**, and an exact **date + time** (12-hour with AM/PM). It pops for the profile at that moment. Button: **Done** (plus Snooze).
